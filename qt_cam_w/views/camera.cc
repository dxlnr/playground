...
void CameraWidget::paintGL() 
{
  ...
  const bool is_mono16 = frame->type == VISION_SENSOR_MONO16;
  const GLenum gl_type = GL_UNSIGNED_BYTE;
  const GLint internal_format = GL_R8;
  const uint8_t *upload_data = frame->data;
  if (is_mono16) {
    const size_t pixel_count = frame->width * frame->height;
    mono8_scratch.resize(pixel_count);
    for (size_t i = 0; i < pixel_count; ++i) {
      const uint16_t value = static_cast<uint16_t>(frame->data[i * 2]) |
                             (static_cast<uint16_t>(frame->data[i * 2 + 1]) << 8);
      mono8_scratch[i] = static_cast<uint8_t>(value >> 8);
    }
    upload_data = mono8_scratch.data();
  }
  if (texture_width != static_cast<int>(frame->width) ||
      texture_height != static_cast<int>(frame->height) ||
      texture_type != frame->type) {
    glBindTexture(GL_TEXTURE_2D, texture_id);
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, frame->width, frame->height, 0, GL_RED, gl_type, nullptr);
    glBindTexture(GL_TEXTURE_2D, 0);
    texture_width = static_cast<int>(frame->width);
    texture_height = static_cast<int>(frame->height);
    texture_type = frame->type;
  }
  glBindTexture(GL_TEXTURE_2D, texture_id);
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, frame->width, frame->height, GL_RED, gl_type, upload_data);
  glBindTexture(GL_TEXTURE_2D, 0);
