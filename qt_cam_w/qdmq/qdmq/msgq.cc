...
#ifdef _WIN32
  uint32_t rnd = distribution(rd);
  DWORD    tid = GetCurrentThreadId();

  uint64_t uid = (static_cast<uint64_t>(rnd) << 32) | tid;
#elif __APPLE__
  // TODO
#else
...
{
  // Buffer must be smaller than 2^32 bytes
  ...
  HANDLE h = CreateFileMappingA(
    ...
    PAGE_READWRITE,
    0,
    static_cast<DWORD>(size + sizeof(msgq_header_t)),
    full_path.c_str());

  if (!h)
    return -1;
  char* mem = (char*)MapViewOfFile(h, FILE_MAP_ALL_ACCESS, 0, 0, size + sizeof(msgq_header_t));
...
void msgq_close_queue(msgq_queue_t *q)
{
  ...
  if (q->hmap && q->hmap != INVALID_HANDLE_VALUE) {
    CloseHandle(q->hmap);
    q->hmap = INVALID_HANDLE_VALUE;
  }
#else
  if (q->mmap_p != NULL)
...
void thread_signal(uint32_t tid) 
{
#ifdef _WIN32
#else
  #ifndef SYS_tkill
...
int msgq_msg_recv(msgq_msg_t * msg, msgq_queue_t * q)
{
 start:
  ...
  if (q->read_uid_local != *q->read_uids[id]) {
    // Reader was evicted, reconnecting
    msgq_init_subscriber(q);
    goto start;
  }
  if (!*q->read_valids[id]) {
...
void msgq_msg_conflate (msgq_msg_t *msg, msgq_queue_t *q) {};
